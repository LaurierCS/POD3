...
public class Spawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (nextEnemyFlySpawnTime < Time.time) {
            Instantiate(enemyFly, (Vector2)transform.position + Random.insideUnitCircle.normalized * enemyFlySpawnDistance, Quaternion.identity);
            nextEnemyFlySpawnTime = Time.time + enemyFlySpawnRate;
        }
        
        // Spawn Worm logic
        if (nextEnemyWormSpawnTime < Time.time) {
            Instantiate(enemyWorm, (Vector2)transform.position + Random.insideUnitCircle.normalized * enemyWormSpawnDistance, Quaternion.identity);
            nextEnemyWormSpawnTime = Time.time + enemyWormSpawnRate;
        }

        /* // Spawn Mantis logic
        if (nextEnemyMantisSpawnTime < Time.time) {
            Instantiate(enemyMantis, (Vector2)transform.position + Random.insideUnitCircle.normalized * enemyMantisSpawnDistance, Quaternion.identity);
